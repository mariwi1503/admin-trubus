...
import { Search, Plus, Edit2, Trash2, Star, Phone, Mail, Download, Loader2, RefreshCw, Cloud, CloudOff, Filter } from 'lucide-react';
      ...
      <div className="flex flex-col gap-4">
          ...
          <div className="flex flex-col sm:flex-row gap-2">
            ...
            <button 
              ...
              className={`flex items-center justify-center gap-2 px-4 py-2.5 border rounded-lg transition-colors h-11 ${
                [filterStatus, filterSpecialization].filter(f => f !== 'all').length > 0 
            ...
            >
              ...
              {[filterStatus, filterSpecialization].filter(f => f !== 'all').length > 0 && (
                <span className="bg-green-600 text-white text-xs px-2 py-0.5 rounded-full ml-1 font-semibold">
                  {[filterStatus, filterSpecialization].filter(f => f !== 'all').length}
                </span>
              )}
            </button>
          </div>
        ...
        {showFilters && (
          <div className="p-4 bg-white border border-gray-100 shadow-sm rounded-xl grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 animate-in fade-in slide-in-from-top-2">
            ...
            <div>
              <label className="block text-xs font-semibold text-gray-500 mb-1.5 uppercase tracking-wider">Spesialisasi</label>
              <select
